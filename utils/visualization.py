...
from mmhuman3d.core.renderer.torch3d_renderer.meshes import (  # noqa: E501
    ParametricMeshes,
)
...
from mmhuman3d.core.renderer.torch3d_renderer.smpl_renderer import SMPLRenderer
from mmhuman3d.core.renderer.torch3d_renderer.utils import (  # noqa: E501
    ...
    if output_path is not None:
        ...
        if image_array is None:
                ...
                # else return frames_folder for reading during rendering.
                    ...
                    if read_frames_batch:
                        ...
                        image_array = images_to_array(
                            origin_frames, img_format=img_format, start=start, end=end
                        )
                        remove_folder = False
                        frames_folder = origin_frames
            ...
            # else return frames_folder for reading during rendering.
                ...
                frames_folder = osp.join(
                    Path(output_path).parent, Path(output_path).name + "_input_temp"
                )
                os.makedirs(frames_folder, exist_ok=True)
                for frame_idx, frame_path in enumerate(frame_list):
                    if check_path_suffix(
                        path_str=frame_path, allowed_suffix=[".jpg", ".png", ".jpeg"]
                    ):
                        shutil.copy(
                            frame_path,
                            os.path.join(frames_folder, "%06d.png" % frame_idx),
                        )
                        img_format = "%06d.png"
                if not read_frames_batch:
                    image_array = images_to_array(
                        frames_folder, img_format=img_format, remove_raw_files=True
                    )
                    frames_folder = None
                    remove_folder = False
                else:
    ...
    if body_model is None:
        if body_model_config is not None:
            ...
            if model_type not in ["smpl", "smplx", "star"]:
                raise ValueError(
                    f"Do not support {model_type}, please choose"
            ...
            if model_path and osp.isdir(model_path):
                ...
                raise FileNotFoundError(
                    "Wrong model_path." " File or directory does not exist."
                )
        else:
        ...
        if body_model_config is not None:
            warnings.warn(
                "Redundant input, will take body_model directly"
...
def _prepare_input_pose(verts, poses, betas, transl):
    ...
    if verts is None and poses is None:
        ...
        warnings.warn(
            "Redundant input, will take verts and ignore poses & " "betas & transl."
        )
        poses = None
        transl = None
        betas = None
...
def _prepare_mesh(poses, betas, transl, verts, start, end, body_model):
    ...
    if poses is not None:
        if isinstance(poses, dict):
            ...
            num_frames = poses["body_pose"].shape[0]
            _, num_person, _ = (
                poses["body_pose"].view(num_frames, -1, NUM_BODY_JOINTS * 3).shape
            )
        ...
        if num_person > 1:
            if betas is not None:
                ...
                warnings.warn(
                    "None betas for multi-person, will all be the "
                    "default body shape."
                )
            ...
            if transl is not None:
                ...
                if transl.shape[1] == 1:
                    ...
                    warnings.warn(
                        f"Transl shape exceed, will be sliced as" f"{transl.shape}."
                    )
                elif transl.shape[1] == num_person:
                    pass
                else:
                ...
                warnings.warn(
                    "None transl for multi-person, will all be the "
                    "default translation."
                )
        ...
        if isinstance(palette, torch.Tensor):
            ...
            for person_idx in range(num_person):
                if palette[person_idx] == "random":
                    color_person = get_different_colors(num_person, int_dtype=False)[
                        person_idx
                    ]
                    ...
                    color_person = (
                        torch.FloatTensor(Color(palette[person_idx]).rgb)
                        .view(1, 1, 3)
                        .repeat(1, num_verts, 1)
                    )
                else:
                    raise ValueError(
                        "Wrong palette string. "
...
def render_smpl(
    ...
    if texture_image is None:
        if isinstance(palette, str):
            ...
            raise ValueError(
                "Wrong input palette type. "
                "Palette should be tensor, array or list of strs"
            )
        colors_all = _prepare_colors(
            palette, render_choice, num_person, num_verts, model_type
        )
    ...
    if mesh_file_path is not None:
        ...
        for person_idx in range(meshes.shape[1]):
            ...
            if texture_image is None:
                ply_paths = [
                    f"{mesh_file_path}/frame{frame_idx}_" f"person{person_idx}.ply"
                ...
                obj_paths = [
                    f"{mesh_file_path}/frame{frame_idx}_" f"person{person_idx}.obj"
                    for frame_idx in range(num_frames)
                ]
...
def visualize_smpl_hmr(
    ...
    **kwargs,
    ...
    K = torch.Tensor(
        get_default_hmr_intrinsic(
            focal_length=focal_length, det_height=det_height, det_width=det_width
        )
    )
    func = partial(
        render_smpl,
    ...
    T = torch.cat(
        [
            cam_transl[..., [1]],
            cam_transl[..., [2]],
            2 * focal_length / (det_width * cam_transl[..., [0]] + 1e-9),
        ],
        -1,
    )
    for k in func.keywords.keys():
    ...
    **kwargs,
    ...
    for k in func.keywords.keys():
        if k in kwargs:
            kwargs.pop(k)
    return func(
        orig_cam=orig_cam, output_path=output_path, resolution=resolution, **kwargs
    )
